...
static void KM_901U_Service(KINGMETER_t* KM_ctx)
{
	...
	case 0:
	    ...
	    if (j<k && l==k+1){
			...
			KM_ctx->Rx.SPEEDMAX_Limit =  KM_ctx->RxBuff[11]; //speedlimit nach boot zuweisen
    ...
    if(KM_ctx->RxState == RXSTATE_DONE)
    {
        ...
        switch(KM_ctx->RxBuff[2])
        {
            case 0x52:      // Operation mode
                ...
                //KM_ctx->Rx.SPEEDMAX_Limit_x10 = (((uint16_t) KM_ctx->RxBuff[7])<<8)  | KM_ctx->RxBuff[6]; //speedlimit old
                KM_ctx->Rx.CUR_Limit_x10      = (((uint16_t) KM_ctx->RxBuff[9])<<8) | KM_ctx->RxBuff[8];
                //KM_ctx->Rx.BATCURRENTMAX=(KM_ctx->RxBuff[8]&0x3F)>>1;
            ...
            case 0x53:      // Settings mode
                ...
                KM_ctx->Rx.SPEEDMAX_Limit =  KM_ctx->RxBuff[j+11]; //speedlimit neu
